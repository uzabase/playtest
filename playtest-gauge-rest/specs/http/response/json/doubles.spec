...
* レスポンスのJSONの"$.key1"が数値の"10.00"である
* レスポンスのJSONの"$.key2[0]"が数値の"0.01"である

## JSON Pathで指定した配列を条件で絞った1要素
* レスポンスボディとしてシナリオデータストアに"{\"key1\":[{\"id\":\"a\",\"key3\":1.0}, {\"id\":\"b\",\"key3\": 2.0 }, {\"id\":\"c\",\"key3\":3.0}]}"を保存する
* レスポンスのJSONの"$.key1"の配列の、UniqueKey"id"の値が"b"である要素の"key3"が、数値の"2.0"である

## 配列内における存在（有）
* レスポンスボディとしてシナリオデータストアに"{\"tests\": [1.1, 2.1, 3.1]}"を保存する
* レスポンスのJSONの配列"$.tests"に、数値"3.1"が存在する

## 配列内において存在（無）
* レスポンスボディとしてシナリオデータストアに"{\"tests\": [1.0, 3.3]}"を保存する
* レスポンスのJSONの配列"$.tests"に、数値"2.1"が存在しない
